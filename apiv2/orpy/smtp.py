...
class SMTPClient:
    ...
    def __init__(self):
        if config("EMAIL_HOST") is None or len(config("EMAIL_HOST")) == 0:
            ...
            self.server = smtplib.SMTP(
                host=config("EMAIL_HOST"), port=config("EMAIL_PORT", cast=int)
            )
        else:
            if len(config("EMAIL_SSL_KEY")) == 0 or len(config("EMAIL_SSL_CERT")) == 0:
                ...
                self.server = smtplib.SMTP_SSL(
                    ...
                    keyfile=config("EMAIL_SSL_KEY"),
                    certfile=config("EMAIL_SSL_CERT"),
                )
    ...
    def __enter__(self):
        if self.server is None:
            return EmptySMTP()
        self.server.ehlo()
        if not config("EMAIL_USE_SSL", cast=bool) and config(
            "EMAIL_USE_TLS", cast=bool
        ):
            self.server.starttls()
            # stmplib docs recommend calling ehlo() before & after starttls()
            self.server.ehlo()
        if (
            len(config("EMAIL_USER", default="")) > 0
            and len(config("EMAIL_PASSWORD", default="")) > 0
        ):
            self.server.login(
    ...
    def test_configuration(self):
        ...
        try:
            ...
            logging.error(
                f'!! SMTP authentication error to {config("EMAIL_HOST")}:{config("EMAIL_PORT", cast=int)}'
            )
            logging.error(e)
            return False, e
