...
func insertNoteEE(db pool.Pool, note *Note, tenantID, projectID, userID uint64) pgx.Row {
	sql := `INSERT INTO sessions_notes (message, user_id, tag, session_id, project_id, timestamp, is_public, thumbnail, start_at, end_at) 
...
func selectSessionNotesEE(db pool.Pool, tenantID, projectID, sessionID, userID uint64) (pgx.Rows, error) {
	sql := `SELECT sessions_notes.note_id, sessions_notes.message, sessions_notes.created_at, 
		       sessions_notes.tag, sessions_notes.session_id, sessions_notes.timestamp, sessions_notes.is_public, 
		       sessions_notes.thumbnail, sessions_notes.start_at, sessions_notes.end_at, users.name AS user_name
			FROM sessions_notes
...
func selectNoteEE(db pool.Pool, tenantID, projectID, userID, noteID uint64) pgx.Row {
	sql := `SELECT sessions_notes.note_id, sessions_notes.message, sessions_notes.created_at, 
		       sessions_notes.tag, sessions_notes.session_id, sessions_notes.timestamp, sessions_notes.is_public, 
		       sessions_notes.thumbnail, sessions_notes.start_at, sessions_notes.end_at, users.name AS user_name
			FROM sessions_notes
...
func selectNoteToShareEE(db pool.Pool, tenantID, projectID, userID, noteID, shareID uint64) pgx.Row {
	sql := `SELECT sessions_notes.note_id, sessions_notes.message, sessions_notes.created_at, 
		       sessions_notes.tag, sessions_notes.session_id, sessions_notes.timestamp, sessions_notes.is_public, 
		       sessions_notes.thumbnail, sessions_notes.start_at, sessions_notes.end_at, users.name AS user_name,
...
func appendExtraConditionEE(conditionsList []string, tenantID uint64) []string {
...
func updateNoteEE(db pool.Pool, tenantID, projectID, userID, noteID uint64, conditionsList []string) pgx.Row {
	sql := fmt.Sprintf(`
		UPDATE sessions_notes SET %s 
        WHERE project_id = $1 AND user_id = $2 AND note_id = $3 AND deleted_at ISNULL
        RETURNING sessions_notes.note_id, sessions_notes.message, sessions_notes.created_at, 
