...
func (c *cacheImpl) Set(project *Project) error {
	...
	if _, err = c.db.Redis.Set(context.Background(), fmt.Sprintf("project:v2:id:%d", project.ProjectID), projectBytes, time.Minute*10).Result(); err != nil {
		return err
	}
	if _, err = c.db.Redis.Set(context.Background(), fmt.Sprintf("project:v2:key:%s", project.ProjectKey), projectBytes, time.Minute*10).Result(); err != nil {
		return err
	}
...
func (c *cacheImpl) GetByID(projectID uint32) (*Project, error) {
	...
	start := time.Now()
	result, err := c.db.Redis.Get(context.Background(), fmt.Sprintf("project:v2:id:%d", projectID)).Result()
	if err != nil {
		return nil, err
	}
...
func (c *cacheImpl) GetByKey(projectKey string) (*Project, error) {
	...
	start := time.Now()
	result, err := c.db.Redis.Get(context.Background(), fmt.Sprintf("project:v2:key:%s", projectKey)).Result()
	if err != nil {
		return nil, err
	}
	c.metrics.RecordRedisRequestDuration(float64(time.Now().Sub(start).Milliseconds()), "get", "project")
