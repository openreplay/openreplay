...
func (r *Rewriter) RewriteCSS(sessionID uint64, baseurl string, css string, externalCfg *externalConfig.Config) string {
	css = rewriteLinks(css, func(rawurl string) string {
		return r.RewriteURL(sessionID, baseurl, rawurl, externalCfg)
	})
	return strings.Replace(css, ":hover", ".-openreplay-hover", -1)
}
