...
export default function (app: App, opts: Partial<Options> = {}) {
  ...
  function sanitize(reqResInfo: RequestResponseData) {
    let finalRequestData = reqResInfo
    if (!options.capturePayload) {
      // @ts-ignore
      delete finalRequestData.request.body
      delete finalRequestData.response.body
    }
    if (options.sanitizer) {
      const resBody = finalRequestData.response.body
      if (typeof resBody === 'string') {
        // Parse response in order to have handy view in sanitization function
        try {
          finalRequestData.response.body = JSON.parse(resBody)
        } catch {}
      }
      finalRequestData = options.sanitizer(finalRequestData) as RequestResponseData
    }
    if (finalRequestData.request.body) {
      finalRequestData.request.body = filterBody(finalRequestData.request.body)
    }
    if (finalRequestData.response.body) {
