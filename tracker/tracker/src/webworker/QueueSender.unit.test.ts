...
describe('QueueSender', () => {
  ...
  // ── split URL param ─────────────────────────────────────────────────────
  // The pair flush in v2 emits concatenated player+assets bytes. The asset
  // start offset is passed as `split` so the server can demux. QueueSender's
  // job is to forward it via &split=N when present.
  describe('split URL param', () => {
    test('push(batch, dataType, split=N) appends &split=N to the fetch URL', () => {
      const queueSender = defaultQueueSender()
      const fetchMock = mockFetch(200)
      queueSender.authorise(randomToken)
      queueSender.push(sampleArray, 'visual', 1234)
      expect(fetchMock).toHaveBeenCalledTimes(1)
      const url = fetchMock.mock.calls[0][0] as string
      expect(url).toContain('&split=1234')
    })

    test('push without split does not add &split= to the URL', () => {
      const queueSender = defaultQueueSender()
      const fetchMock = mockFetch(200)
      queueSender.authorise(randomToken)
    ...
    test('sendUncompressed forwards split via &split=', () => {
      const queueSender = defaultQueueSender()
      const fetchMock = mockFetch(200)
      queueSender.authorise(randomToken)
      queueSender.sendUncompressed(sampleArray, 'visual', 999)
      const url = fetchMock.mock.calls[0][0] as string
      expect(url).toContain('&split=999')
    })

    test('sendCompressed forwards split via &split=', () => {
      const queueSender = defaultQueueSender()
      const fetchMock = mockFetch(200)
      queueSender.authorise(randomToken)
      queueSender.sendCompressed(sampleArray, 'visual', 42)
      const url = fetchMock.mock.calls[0][0] as string
      expect(url).toContain('&split=42')
    })

    test('queued entry preserves split until it is sent', () => {
      const queueSender = defaultQueueSender()
      const fetchMock = mockFetch(200)
      // Push before authorise → entry queues with its split.
      queueSender.push(sampleArray, 'visual', 777)
      expect(fetchMock).not.toHaveBeenCalled()
      queueSender.authorise(randomToken)
      expect(fetchMock).toHaveBeenCalledTimes(1)
      const url = fetchMock.mock.calls[0][0] as string
      expect(url).toContain('&split=777')
    })
  })
})
