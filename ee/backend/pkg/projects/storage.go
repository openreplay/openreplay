...
func (c *projectsImpl) getProjectByKey(projectKey string) (*Project, error) {
	...
	if err := c.db.QueryRow(`
		SELECT project_id, name, tenant_id, active, max_session_duration, save_request_payloads, sample_rate, beacon_size, platform,
	...
	`,
		projectKey,
	).Scan(&p.ProjectID, &p.Name, &p.TenantID, &p.Active, &p.MaxSessionDuration, &p.SaveRequestPayloads, &p.SampleRate, &p.BeaconSize, &p.Platform,
...
func (c *projectsImpl) getProject(projectID uint32) (*Project, error) {
	...
	if err := c.db.QueryRow(`
		SELECT project_key, name, tenant_id, active, max_session_duration, save_request_payloads, sample_rate, beacon_size, platform,
			metadata_1, metadata_2, metadata_3, metadata_4, metadata_5,
			metadata_6, metadata_7, metadata_8, metadata_9, metadata_10
		FROM projects
	...
	`,
		projectID,
	).Scan(&p.ProjectKey, &p.Name, &p.TenantID, &p.Active, &p.MaxSessionDuration, &p.SaveRequestPayloads, &p.SampleRate, &p.BeaconSize, &p.Platform,
		&p.Metadata1, &p.Metadata2, &p.Metadata3, &p.Metadata4, &p.Metadata5,
		&p.Metadata6, &p.Metadata7, &p.Metadata8, &p.Metadata9, &p.Metadata10); err != nil {
		return nil, err
	}
...
func (c *projectsImpl) listProjectsByTenantID(tenantID int) ([]*Project, error) {
	rows, err := c.db.Query(`
		SELECT project_id, project_key, name, tenant_id, active, max_session_duration, save_request_payloads, sample_rate, beacon_size, platform,
			metadata_1, metadata_2, metadata_3, metadata_4, metadata_5,
			metadata_6, metadata_7, metadata_8, metadata_9, metadata_10
		FROM projects
	...
	for rows.Next() {
		p := &Project{}
		if err := rows.Scan(&p.ProjectID, &p.ProjectKey, &p.Name, &p.TenantID, &p.Active, &p.MaxSessionDuration, &p.SaveRequestPayloads, &p.SampleRate, &p.BeaconSize, &p.Platform,
			&p.Metadata1, &p.Metadata2, &p.Metadata3, &p.Metadata4, &p.Metadata5,
			&p.Metadata6, &p.Metadata7, &p.Metadata8, &p.Metadata9, &p.Metadata10); err != nil {
			return nil, err
		}
