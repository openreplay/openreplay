...
from chalicelib.core.dashboard import __get_basic_constraints, __get_constraint_values
...
def journey(project_id, startTimestamp=TimeUTC.now(delta_days=-1), endTimestamp=TimeUTC.now(), filters=[], **args):
    pg_sub_query_subset = __get_basic_constraints(project_id=project_id, data=args, duration=True,
                                                  main_table="sessions",
...
def users_retention(project_id, startTimestamp=TimeUTC.now(delta_days=-70), endTimestamp=TimeUTC.now(), filters=[],
    ...
    pg_sub_query = __get_basic_constraints(project_id=project_id, data=args, duration=True, main_table="sessions",
                                           time_constraint=True)
...
def feature_adoption_daily_usage(project_id, startTimestamp=TimeUTC.now(delta_days=-70), endTimestamp=TimeUTC.now(),
    ...
    pg_sub_query = __get_basic_constraints(project_id=project_id, data=args, duration=True, main_table="sessions",
                                           time_constraint=True)
    pg_sub_query_chart = __get_basic_constraints(project_id=project_id, time_constraint=True,
...
def users_power(project_id, startTimestamp=TimeUTC.now(delta_days=-70), endTimestamp=TimeUTC.now(),
                filters=[], **args):
    pg_sub_query = __get_basic_constraints(project_id=project_id, time_constraint=True, chart=False, data=args)
    pg_sub_query.append("user_id IS NOT NULL")
...
def search(text, feature_type, project_id, platform=None):
    ...
    pg_sub_query = __get_basic_constraints(project_id=project_id, time_constraint=True, duration=True,
