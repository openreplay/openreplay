...
async def reset(data: schemas.ForgetPasswordPayloadSchema):
    ...
    if a_users:
        invitation_link = await users.generate_new_invitation(user_id=a_users["userId"])
        await email_helper.send_forgot_password(recipient=data.email, invitation_link=invitation_link)
    else:
