...
async def events_search(projectId: int, q: str,
    ...
    if len(q) == 0:
        return {"data": []}
    if live:
        out = await assist.autocomplete(project_id=projectId, q=q,
...
async def get_all_sentry(context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_sentry.get_all(tenant_id=context.tenant_id)
    return {"data": data}
...
async def get_sentry(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_sentry.get(project_id=projectId)
    return {"data": data}
...
async def add_edit_sentry(projectId: int, data: schemas.IntegrationSentrySchema = Body(...),
                    context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_sentry.add_edit(tenant_id=context.tenant_id, project_id=projectId, data=data)
    return {"data": data}
...
async def delete_sentry(projectId: int, _=Body(None), context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_sentry.delete(tenant_id=context.tenant_id, project_id=projectId)
    return {"data": data}
...
async def get_all_datadog(context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_datadog.get_all(tenant_id=context.tenant_id)
    return {"data": data}
...
async def get_datadog(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_datadog.get(project_id=projectId)
    return {"data": data}
...
async def add_edit_datadog(projectId: int, data: schemas.IntegrationDatadogSchema = Body(...),
                     context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_datadog.add_edit(tenant_id=context.tenant_id, project_id=projectId, data=data)
    return {"data": data}
...
async def delete_datadog(projectId: int, _=Body(None), context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_datadog.delete(tenant_id=context.tenant_id, project_id=projectId)
    return {"data": data}
...
async def get_all_stackdriver(context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_stackdriver.get_all(tenant_id=context.tenant_id)
    return {"data": data}
...
async def get_stackdriver(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_stackdriver.get(project_id=projectId)
    return {"data": data}
...
async def add_edit_stackdriver(projectId: int, data: schemas.IntegartionStackdriverSchema = Body(...),
                         context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_stackdriver.add_edit(tenant_id=context.tenant_id, project_id=projectId, data=data)
    return {"data": data}
...
async def delete_stackdriver(projectId: int, _=Body(None), context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_stackdriver.delete(tenant_id=context.tenant_id, project_id=projectId)
    return {"data": data}
...
async def get_all_newrelic(context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_newrelic.get_all(tenant_id=context.tenant_id)
    return {"data": data}
...
async def get_newrelic(projectId: int, context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_newrelic.get(project_id=projectId)
    return {"data": data}
...
async def add_edit_newrelic(projectId: int, data: schemas.IntegrationNewrelicSchema = Body(...),
                      context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_newrelic.add_edit(tenant_id=context.tenant_id, project_id=projectId, data=data)
    return {"data": data}
...
async def delete_newrelic(projectId: int, _=Body(None), context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_newrelic.delete(tenant_id=context.tenant_id, project_id=projectId)
    return {"data": data}
...
async def get_all_rollbar(context: schemas.CurrentContext = Depends(OR_context)):
    data = await log_tool_rollbar.get_all(tenant_id=context.tenant_id)
    return {"data": data}
...
async def add_edit_jira_cloud(data: schemas.IssueTrackingJiraSchema = Body(...),
                        context: schemas.CurrentContext = Depends(OR_context)):
...
async def update_user_module(context: schemas.CurrentContext = Depends(OR_context),
    ...
    return await users.update_user_module(context.user_id, data)
...
async def get_slack_webhook(webhookId: int, context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await Slack.get_integration(tenant_id=context.tenant_id, integration_id=webhookId)}
...
async def edit_msteams_integration(webhookId: int, data: schemas.EditCollaborationSchema = Body(...),
    ...
    if len(data.url) > 0:
        ...
        if old["endpoint"] != data.url:
            if not await MSTeams.say_hello(data.url):
                return {
                    "errors": [
                        "We couldn't send you a test message on your Microsoft Teams channel. Please verify your webhook url."]
                }
...
async def get_assist_stats_avg(
        project_id: int,
        startTimestamp: int = None,
        endTimestamp: int = None,
        userId: str = None
):
    return await assist_stats.get_averages(
...
async def get_assist_stats_top_members(
    ...
    return await assist_stats.get_top_members(
        project_id=project_id,
        start_timestamp=startTimestamp,
        end_timestamp=endTimestamp,
        sort_by=sort,
...
async def get_assist_stats_sessions(
    ...
    return await assist_stats.get_sessions(
        project_id=project_id,
        data=data
    )
