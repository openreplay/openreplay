...
@public_app.get("/version", tags=["operational"])
def get_version():
    return {
        "version": config("OPENREPLAY_VERSION"),
    }
...
if not tenants.tenants_exists_sync(use_pool=False):
    ...
    async def signup_handler(
        response: JSONResponse, data: schemas.UserSignupSchema = Body(...)
    ):
...
def __process_authentication_response(response: JSONResponse, data: dict) -> dict:
    ...
    data = {
        ...
        "data": {"scopeState": scope.get_scope(-1), "user": data},
    }
    response.set_cookie(
        key="refreshToken",
        value=refresh_token,
    ...
    response.set_cookie(
        key="spotRefreshToken",
        value=spot_refresh_token,
        path=spot.COOKIE_PATH,
        max_age=spot_refresh_token_max_age,
        secure=True,
        httponly=True,
    )
    return data
...
def login_user(response: JSONResponse, data: schemas.UserLoginSchema = Body(...)):
    if helper.allow_captcha() and not captcha.is_valid(data.g_recaptcha_response):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid captcha."
        )
    ...
    if "errors" in r:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=r["errors"][0]
        )
...
def logout_user(
    response: Response, context: schemas.CurrentContext = Depends(OR_context)
):
...
def refresh_login(
    response: JSONResponse, context: schemas.CurrentContext = Depends(OR_context)
):
...
def edit_account(
    data: schemas.EditAccountSchema = Body(...),
    context: schemas.CurrentContext = Depends(OR_context),
):
    return users.edit_account(
        tenant_id=context.tenant_id, user_id=context.user_id, changes=data
    )


@app.post("/account/scope", tags=["account"])
def change_scope(
    data: schemas.ScopeSchema = Body(),
    context: schemas.CurrentContext = Depends(OR_context),
):
...
def change_client_password(
    resqponse: JSONResponse,
    data: schemas.EditUserPasswordSchema = Body(...),
    context: schemas.CurrentContext = Depends(OR_context),
):
    r = users.change_password(
        email=context.email,
...
def edit_slack_integration(
    ...
    if len(data.url) > 0:
        old = Slack.get_integration(
            tenant_id=context.tenant_id, integration_id=integrationId
        )
        if not old:
...
def process_invitation_link(token: str):
    ...
    if user["expiredInvitation"]:
        return {
            "errors": ["expired invitation, please ask your admin to send a new one"]
        }
    if (
        user["expiredChange"] is not None
        and not user["expiredChange"]
        and user["changePwdToken"] is not None
        and user["changePwdAge"] < -5 * 60
    ):
        pass_token = user["changePwdToken"]
    else:
        pass_token = users.allow_password_change(user_id=user["userId"])
    return RedirectResponse(
        url=config("SITE_URL") + config("change_password_link") % (token, pass_token)
    )


@public_app.post("/password/reset", tags=["users"])
def change_password_by_invitation(
    response: JSONResponse, data: schemas.EditPasswordByInvitationSchema = Body(...)
):
    if data is None or len(data.invitation) < 64 or len(data.passphrase) < 8:
        return {"errors": ["please provide a valid invitation & pass"]}
    user = users.get_by_invitation_token(
        token=data.invitation, pass_token=data.passphrase
    )
    if user is None:
        return {"errors": ["invitation not found"]}
    ...
    r = users.set_password_invitation(
        new_password=data.password.get_secret_value(), user_id=user["userId"]
    )
    r = __process_authentication_response(response=response, data=r)
    return r
...
def edit_member(
    memberId: int,
    data: schemas.EditMemberSchema,
    context: schemas.CurrentContext = Depends(OR_context),
):
    return users.edit_member(
        tenant_id=context.tenant_id,
...
def search_sessions_by_metadata(
    ...
    return {
        "data": sessions.search_by_metadata(
            tenant_id=context.tenant_id,
            user_id=context.user_id,
            m_value=value,
            m_key=key,
...
def get_projects(context: schemas.CurrentContext = Depends(OR_context)):
    return {
        "data": projects.get_projects(
            tenant_id=context.tenant_id, gdpr=True, recorded=True
        )
    }
...
@app.get(
    "/{projectId}/sessions/{sessionId}/errors/{errorId}/sourcemaps",
    tags=["sessions", "sourcemaps"],
)
def get_error_trace(
    projectId: int,
    sessionId: int,
...
def errors_get_details_sourcemaps(
    projectId: int, errorId: str, context: schemas.CurrentContext = Depends(OR_context)
):
    data = errors.get_trace(project_id=projectId, error_id=errorId)
    if "errors" in data:
        return data
...
def add_remove_favorite_error(
    ...
    if action == "favorite":
        return errors_favorite.favorite_error(
            project_id=projectId, user_id=context.user_id, error_id=errorId
        )
...
def get_live_session_replay_file(
    projectId: int,
    sessionId: Union[int, str],
    context: schemas.CurrentContext = Depends(OR_context),
):
    not_found = {"errors": ["Replay file not found"]}
    sessionId, err = unprocessed_sessions.check_exists(
        project_id=projectId, session_id=sessionId, not_found_response=not_found
    )
    if err is not None:
        return err
...
def get_live_session_devtools_file(
    projectId: int,
    sessionId: Union[int, str],
    context: schemas.CurrentContext = Depends(OR_context),
):
...
def get_heatmaps_by_session_id_url(
    ...
    return {
        "data": heatmaps.get_x_y_by_url_and_session_id(
            project_id=projectId, session_id=sessionId, data=data
        )
    }
...
def add_remove_favorite_session2(
    projectId: int,
    sessionId: int,
    context: schemas.CurrentContext = Depends(OR_context),
):
    return sessions_favorite.favorite_session(
        context=context, project_id=projectId, session_id=sessionId
    )


@app.get("/{projectId}/sessions/{sessionId}/assign", tags=["sessions"])
def assign_session(
    projectId: int, sessionId, context: schemas.CurrentContext = Depends(OR_context)
):
    data = sessions_assignments.get_by_session(
        project_id=projectId,
        session_id=sessionId,
        tenant_id=context.tenant_id,
        user_id=context.user_id,
...
@app.get(
    "/{projectId}/sessions/{sessionId}/assign/{issueId}",
    tags=["sessions", "issueTracking"],
)
def assign_session(
    projectId: int,
    sessionId: int,
...
@app.post(
    "/{projectId}/sessions/{sessionId}/assign/{issueId}/comment",
    tags=["sessions", "issueTracking"],
)
...
def get_session_notes(
    ...
    data = sessions_notes.get_session_notes(
        tenant_id=context.tenant_id,
        project_id=projectId,
        session_id=sessionId,
        user_id=context.user_id,
...
def share_note_to_slack(
    projectId: int,
    noteId: int,
    webhookId: int,
    context: schemas.CurrentContext = Depends(OR_context),
):
    return sessions_notes.share_to_slack(
        tenant_id=context.tenant_id,
        project_id=projectId,
        user_id=context.user_id,
        note_id=noteId,
...
def share_note_to_msteams(
    projectId: int,
    noteId: int,
    webhookId: int,
    context: schemas.CurrentContext = Depends(OR_context),
):
    return sessions_notes.share_to_msteams(
        tenant_id=context.tenant_id,
        project_id=projectId,
        user_id=context.user_id,
        note_id=noteId,
...
def get_all_notes(
    projectId: int,
    data: schemas.SearchNoteSchema = Body(...),
    context: schemas.CurrentContext = Depends(OR_context),
):
    data = sessions_notes.get_all_notes_by_project_id(
        tenant_id=context.tenant_id,
        project_id=projectId,
        user_id=context.user_id,
        data=data,
...
def search_feature_flags(
    ...
    return feature_flags.search_feature_flags(
        project_id=project_id, user_id=context.user_id, data=data
    )
...
def get_feature_flag(project_id: int, feature_flag_id: int):
    return feature_flags.get_feature_flag(
...
def add_feature_flag(
    ...
    return feature_flags.create_feature_flag(
        project_id=project_id, user_id=context.user_id, feature_flag_data=data
    )
...
def update_feature_flag(
    ...
    return feature_flags.update_feature_flag(
...
def delete_feature_flag(project_id: int, feature_flag_id: int, _=Body(None)):
    return {
        "data": feature_flags.delete_feature_flag(
            project_id=project_id, feature_flag_id=feature_flag_id
        )
    }
...
@app.post(
    "/{project_id}/feature-flags/{feature_flag_id}/status", tags=["feature flags"]
)
def update_feature_flag_status(
    project_id: int, feature_flag_id: int, data: schemas.FeatureFlagStatus = Body(...)
):
    return {
        "data": feature_flags.update_feature_flag_status(
            project_id=project_id,
            feature_flag_id=feature_flag_id,
