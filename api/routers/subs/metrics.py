...
async def try_card(projectId: int, data: schemas.CardSchema = Body(...),
             context: schemas.CurrentContext = Depends(OR_context)):
    return {"data": await custom_metrics.get_chart(project_id=projectId, data=data, user_id=context.user_id)}
...
async def try_card_sessions(projectId: int, data: schemas.CardSessionsSchema = Body(...),
                      context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.get_sessions(project_id=projectId, user_id=context.user_id, data=data)
    return {"data": data}


@app.post('/{projectId}/cards/try/issues', tags=["cards"])
async def try_card_issues(projectId: int, data: schemas.CardSchema = Body(...),
                    context: schemas.CurrentContext = Depends(OR_context)):
...
async def get_card_sessions(projectId: int, metric_id: int,
                      ...
                      context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.get_sessions_by_card_id(project_id=projectId, user_id=context.user_id, metric_id=metric_id,
...
async def get_card_errors_list(projectId: int, metric_id: int,
                         data: schemas.CardSessionsSchema = Body(...),
                         context: schemas.CurrentContext = Depends(OR_context)):
    data = await custom_metrics.get_errors_list(project_id=projectId, user_id=context.user_id,
...
async def get_card_chart(projectId: int, metric_id: int, request: Request, data: schemas.CardSessionsSchema = Body(...),
                   context: schemas.CurrentContext = Depends(OR_context)):
...
async def update_card(projectId: int, metric_id: int, data: schemas.CardSchema = Body(...),
                context: schemas.CurrentContext = Depends(OR_context)):
...
async def update_card_state(projectId: int, metric_id: int,
...
async def delete_card(projectId: int, metric_id: int, _=Body(None),
                context: schemas.CurrentContext = Depends(OR_context)):
