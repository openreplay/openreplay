...
def get_favorite_sessions(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return {
...
def get_session2(projectId: int, sessionId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def assign_session(projectId: int, sessionId, context: schemas.CurrentContext = Depends(JWTBearer())):
...
# def sessions_search2(projectId:int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def session_filter_values(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return {'data': sessions_metas.get_key_values(projectId)}


@app.get('/{projectId}/sessions/filters/top', tags=["sessions"])
def session_top_filter_values(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return {'data': sessions_metas.get_top_key_values(projectId)}


@app.get('/{projectId}/sessions/filters/search', tags=["sessions"])
def get_session_filters_meta(projectId: int, q: str, type: str, context: schemas.CurrentContext = Depends(JWTBearer())):
    meta_type = type
    if len(meta_type) == 0:
        return {"data": []}
    if len(q) == 0:
...
def get_all_sentry(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": log_tool_sentry.get_all(tenant_id=context.tenant_id)}


@app.get('/{projectId}/integrations/sentry', tags=["integrations"])
def get_sentry(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_sentry(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def proxy_sentry(projectId: int, eventId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_datadog(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": log_tool_datadog.get_all(tenant_id=context.tenant_id)}


@app.get('/{projectId}/integrations/datadog', tags=["integrations"])
def get_datadog(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_datadog(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_stackdriver(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_stackdriver(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_stackdriver(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_newrelic(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_newrelic(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_newrelic(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_rollbar(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": log_tool_rollbar.get_all(tenant_id=context.tenant_id)}


@app.get('/{projectId}/integrations/rollbar', tags=["integrations"])
def get_rollbar(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_bugsnag(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": log_tool_bugsnag.get_all(tenant_id=context.tenant_id)}


@app.get('/{projectId}/integrations/bugsnag', tags=["integrations"])
def get_bugsnag(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_bugsnag(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_cloudwatch(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_cloudwatch(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_cloudwatch(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_elasticsearch(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_elasticsearch(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_elasticsearch(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_sumologic(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_sumologic(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_sumologic(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_integration_status(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_default_issue_tracking_tool(context: schemas.CurrentContext = Depends(JWTBearer())):
    error, integration = integrations_manager.get_integration(tenant_id=context.tenant_id,
                                                              user_id=context.user_id)
    if error is not None:
        return error
...
def delete_jira_cloud(context: schemas.CurrentContext = Depends(JWTBearer())):
    error, integration = integrations_manager.get_integration(tool=integration_jira_cloud.PROVIDER,
                                                              tenant_id=context.tenant_id,
                                                              user_id=context.user_id)
    if error is not None:
...
def delete_github(context: schemas.CurrentContext = Depends(JWTBearer())):
    error, integration = integrations_manager.get_integration(tool=integration_github.PROVIDER,
                                                              tenant_id=context.tenant_id,
                                                              user_id=context.user_id)
    if error is not None:
...
def get_all_issue_tracking_projects(context: schemas.CurrentContext = Depends(JWTBearer())):
    error, integration = integrations_manager.get_integration(tenant_id=context.tenant_id,
                                                              user_id=context.user_id)
    if error is not None:
        return error
...
def get_integration_metadata(integrationProjectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    error, integration = integrations_manager.get_integration(tenant_id=context.tenant_id,
                                                              user_id=context.user_id)
    if error is not None:
        return error
...
def get_all_assignments(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_gdpr(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_metadata(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_metadata(projectId: int, index: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def search_metadata(projectId: int, q: str, key: str, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def search_integrations(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_capture_status(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_all_announcements(context: schemas.CurrentContext = Depends(JWTBearer())):
...
# def errors_merge(projectId:int, context: schemas.CurrentContext = Depends(JWTBearer())):
#     data = app.current_request.json_body
#
...
def errors_merge(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": False}


#
# @app.route('/{projectId}/alerts', methods=['POST', 'PUT'])
# def create_alert(projectId:int, context: schemas.CurrentContext = Depends(JWTBearer())):
#     data = app.current_request.json_body
#     return alerts.create(projectId:int, data)
#
#
@app.get('/{projectId}/alerts', tags=["alerts"])
def get_all_alerts(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": alerts.get_all(projectId)}


@app.get('/{projectId}/alerts/{alertId}', tags=["alerts"])
def get_alert(projectId: int, alertId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": alerts.get(alertId)}


# @app.route('/{projectId}/alerts/{alertId}', methods=['POST', 'PUT'])
# def update_alert(projectId:int, alertId, context: schemas.CurrentContext = Depends(JWTBearer())):
#     data = app.current_request.json_body
#     return alerts.update(alertId, data)
#
#
@app.delete('/{projectId}/alerts/{alertId}', tags=["alerts"])
def delete_alert(projectId: int, alertId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
    return alerts.delete(projectId, alertId)


#
# @app.route('/{projectId}/funnels', methods=['POST', 'PUT'])
# def add_funnel(projectId:int, context: schemas.CurrentContext = Depends(JWTBearer())):
#     data = app.current_request.json_body
...
def get_funnels(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_possible_issue_types(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_funnel_sessions(projectId: int, funnelId: int, rangeValue: str = None, startDate: int = None,
                        endDate: int = None, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def get_funnel(projectId: int, funnelId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def delete_filter(projectId: int, funnelId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
# def sign_sourcemap_for_upload(projectId:int, context: schemas.CurrentContext = Depends(JWTBearer())):
#     data = app.current_request.json_body
...
def get_weekly_report_config(context: schemas.CurrentContext = Depends(JWTBearer())):
...
def issue_types(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
...
def all_issue_types(context: schemas.CurrentContext = Depends(JWTBearer())):
    return {"data": issues.get_all_types()}


@app.get('/{projectId}/assist/sessions', tags=["assist"])
def sessions_live(projectId: int, context: schemas.CurrentContext = Depends(JWTBearer())):
