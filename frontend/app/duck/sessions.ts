  ...
  switch (action.type) {
    case FETCH_ERROR_STACK.SUCCESS:
      return state
        .set(
          'errorStack',
          List(action.data.trace).map((es) => new ErrorStack(es))
        )
        .set('sourcemapUploaded', action.data.sourcemapUploaded);
    case FETCH_LIVE_LIST.SUCCESS:
      const liveList = action.data.sessions.map(
        (s) => new Session({ ...s, live: true })
      );
    ...
    case FETCHV2.SUCCESS: {
      ...
      return state.set('current', session).set('prefetched', false);
    }
    case PREFETCH_SESSION: {
      const { data } = action;
      return state.set('current', data).set('prefetched', true);
    }
    ...
    case FETCH_FAVORITE_LIST.SUCCESS:
      return state.set(
        'favoriteList',
        action.data.map((s) => new Session(s))
      );
    case TOGGLE_FAVORITE.SUCCESS: {
      ...
      const wasInFavorite =
        state
          .get('favoriteList')
          .findIndex(({ sessionId }) => sessionId === id) > -1;

      if (session && !wasInFavorite) {
        session.favorite = true;
        mutableState = mutableState.updateIn(
          ['list', sessionIdx],
          () => session
        );
      }
      if (current.sessionId === id) {
        mutableState = mutableState.update('current', (s: Session) => ({
          ...s,
          favorite: !wasInFavorite,
        }));
      }
      return mutableState.update('favoriteList', (list: Session[]) =>
        session
          ? wasInFavorite
    ...
    case SORT: {
      ...
      return state
        .update('list', (list: Session[]) => list.sort(comparator))
        .update('favoriteList', (list: Session[]) => list.sort(comparator));
    }
    case SET_ACTIVE_TAB:
      const allList =
        action.tab.type === 'all'
          ...
          : state
              .get('list')
              .filter((s) => s.issueTypes.includes(action.tab.type));

      return state.set('activeTab', action.tab).set(
        'sessionIds',
        allList.map(({ sessionId }) => sessionId)
      );
    case SET_TIMEZONE:
    ...
    case FILTER_OUT_NOTE:
      return state.updateIn(['current'], (session: Session) => ({
        ...session,
    ...
    case ADD_NOTE:
      const session = state.get('current') as Session;
      session.notesWithEvents = [...session.notesWithEvents, action.note].sort(
        (a, b) => {
    ...
    case UPDATE_NOTE:
      ...
      const noteIndex = currSession.notesWithEvents.findIndex(
        (item) => item.noteId === action.note.noteId
      );
    ...
    case LAST_PLAYED_SESSION_ID:
      ...
      const sIndex = sessionList.findIndex(
        ({ sessionId }) => sessionId === action.sessionId
      );
      if (sIndex === -1) return state;

      return state.updateIn(['list', sIndex], (session: Session) => ({
        ...session,
        viewed: true,
      }));
    default:
...
export const fetchList =
  ...
  (dispatch) => {
    if (!force) {
      // compare with the last fetched filter
...
export function fetchErrorStackList(sessionId, errorId) {
  return {
    ...
    call: (client) =>
      client.get(`/sessions/${sessionId}/errors/${errorId}/sourcemaps`),
  };
}
...
export const fetch =
  ...
  (dispatch, getState) => {
    dispatch({
      ...
      call: (client) =>
        client.get(
          isLive ? `/assist/sessions/${sessionId}` : `/sessions/${sessionId}`
        ),
...
export const fetchV2 = (sessionId: string) => (dispatch, getState) => {
  const apiClient = new APIClient();
  const apiGet = (url: string, dispatch: any, FAILURE?: string) =>
    apiClient
      .get(url)
      .then(async (response) => {
      ...
      .catch(async (e) => {
        ...
        return (
          ...
          dispatch({
            type: FAILURE,
            errors: data ? parseError(data.errors) : [],
          })
        );
      });

  const filter = getState().getIn(['filters', 'appliedFilter']);
  apiGet(`/sessions/${sessionId}/replay`, dispatch, FETCH.FAILURE).then(
    async ({ jwt, errors, data }) => {
      if (errors) {
...
export const setCustomSession = (session, filter) => (dispatch, getState) => {
  dispatch({
    type: FETCH.SUCCESS,
    filter: getState().getIn(['filters', 'appliedFilter']),
    data: session,
