...
const reducer = (state = initialState, action: IAction) => {
  switch (action.type) {
    ...
    case FETCH_EVENTS.SUCCESS: {
      const { errors, events, issues, resources, stackEvents, userEvents } = action.data as {
        errors: any[];
        events: any[];
        issues: any[];
        resources: any[];
    ...
    case TOGGLE_FAVORITE.SUCCESS: {
      ...
      const wasInFavorite =
        state.get('favoriteList').findIndex(({ sessionId }) => sessionId === id) > -1;
...
export const fetch =
  ...
  (dispatch, getState) => {
    dispatch({
      types: FETCH.toArray(),
      call: (client) =>
        client.get(isLive ? `/assist/sessions/${sessionId}` : `/sessions/${sessionId}`),
