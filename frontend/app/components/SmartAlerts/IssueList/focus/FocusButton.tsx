...
/* Focus entry point: a header button that wears the blue active-tag look while
   anything is focused, opening a Yours / Team popover — a switch per focus
   (anyone can toggle: shared analysis budget), edit/delete on your own,
   + New focus. NOT-YET-BACKED (focuses hydrate empty until the API ships). */

const FocusRow = observer(
  ({ focus, onEdit }: { focus: Focus; onEdit: (f: Focus) => void }) => {
      ...
      <div className="flex items-center gap-3 py-2 px-2 -mx-2 rounded-lg group hover:bg-gray-lightest transition-colors">
        ...
        <div className="flex flex-col min-w-0 flex-1">
          ...
          <Tooltip title={focus.summary} placement="left">
            <span className="text-xs truncate cursor-default color-gray-medium">
              ~{focus.trafficPct}% · ~{focus.sessionsPerDay.toLocaleString()}{' '}
              {t('sessions/day')}
              {!focus.mine && ` · ${t('by')} ${focus.createdBy}`}
            </span>
          </Tooltip>
        </div>
        ...
        <span className="w-6 shrink-0 flex items-center justify-center">
          {focus.mine && (
            <Dropdown
              ...
              menu={{
                items: [
                  { key: 'edit', icon: <Pencil size={14} />, label: t('Edit') },
    ...
    <div className="flex flex-col" style={{ width: 340 }}>
      <div className="pb-2.5 border-b -mx-1 px-1">
        <div className="flex items-center gap-1.5">
          ...
          <Tooltip
            placement="bottom"
            title={t(
              'Full traffic keeps a baseline share of the daily sample; active focuses get concentrated sampling on top. Anyone can switch a focus on or off.',
            )}
          >
        ...
        <div className="text-xs mt-0.5 color-gray-medium">
          {t("Concentrate the agent's analysis on portions of your traffic.")}
        </div>
      </div>
      ...
      {issuesStore.focuses.length === 0 ? (
        <div className="text-sm py-3 color-gray-medium">
          {t(
            'No focuses yet — the agent samples all traffic. Create one to concentrate its analysis on the part you care about.',
          )}
        </div>
      ) : (
        <>
          {mine.length > 0 && sectionTitle(t('Yours'))}
          {mine.map((f) => (
